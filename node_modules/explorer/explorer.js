...
function message(ws, content)
{
   ws.emit('message', { 'content' : content });
}
...
{
	console.log("Getdir currentpath unresolved: " + thepath + " target: " + params.target);  
  ...
  path.exists(thepath, function (result) { 
    if(!result) { 
      websocket.emit('message', { 'content' : "Dir not found " + thepath   }); 
      return;
    }
  ...
  fs.readdir(thepath, function(err, arrfiles)
    ...
    console.log("thepath " + thepath+ ", " + arrfiles.length + " files");
      ...
      var plural = sumfile > 1 ? 's' : '';
      message =  thepath + ', ' + sumfile + " file"+ plural + ", " + sumsize + " bytes."; 
      websocket.emit('status', {'content' : message });
		});
	 });  // readdir
  }); // exists
}
...
function readImage(websocket, fs, filepath)
{
  ...
  path.exists(filepath, function(result) { 
    	fs.readFile(filepath, function(err, file)
      { 
         console.log("Request completed.");
         websocket.emit('image', { 'ext': ext, 'path': filepath, 'content' : new Buffer(file).toString('base64')});
         return file;                   
      }
      );
    });
...
function mkdir(websocket, cpath, newdir)
{
  ...
  fs.mkdir(p, '0777', function(err){
        if(err) websocket.emit('message', { 
          'content' : "Enable to create " + p
        }); 
  });
}
  ...
  fs.rename(oldname, newname, function(err){
    ...
    { 
      websocket.emit('message', { 
        'content' : "Enable to rename " + oldname + " into " + newname
      });
      return;  
    }       
...
function deletelist(websocket, fs, params)
{
  ...
  console.log("Deleted");
  websocket.emit('notification',  { 'action': 'update', 'target' : target } );
  websocket.emit('status', {'content' : message });
}
...
function copyIf(ws, sourcepath, targetpath, promptdup) 
{
  ...
  if(promptdup)
  {
    if(fs.existsSync(targetpath))
    {
      ...
      ws.emit('confirm', {'question' : targetpath + " already exists. Overwrite?" },
        function(data){
...
function copysub(websocket, flist, source, target)
{
  for(var i = 0; i < flist.length; i++)
  {
    ...
    var info = "Copying "+ sourcepath + " to " + targetpath; 
    console.log(info);
    websocket.emit('status',  { 'content': info } );
...
function filecopy(websocket, fs, params)
{
  ...
  websocket.emit('notification',  { 'action': 'update', 'target' : params.target } );
  websocket.emit('status',  { 'content': selection.length + ' file' + s + ' copied.' } );
...
function copyrename(websocket, fs, params)
{
  ...
  websocket.emit('status',  { 'content': oldname + " copied as " + newname } );
  websocket.emit('notification',  { 'action': 'update', 'target' : params.target } );    
}
...
function getcontent(websocket, fs, params)
{
  ...
  if(target != null)
	...
	fs.exists(fullpath, function(here) { 
		if(here)
		{
      var stats = fs.statSync(fullpath);     
      if(stats.isDirectory())
      {
         websocket.emit('message',  { 'content': "Can't edit a directory." } );
         return;        
      }    
      ...
      if(data.length >= 127)
      {
        ...
        //console.log("Content formatted: " + data);
      }
      ...
      //console.log("Data:" + data + " Ext:" + ft);      
   		websocket.emit('editor', { 'content' : { 'content': data, 'filetype' :ft, 'filename': fullpath }   });
...
function savefile(websocket, fs, params)
{
  ...
  if(!params.overwrite)
  {
    ...
    { 
      ...
      if(stats.isDirectory())
      {
        websocket.emit('editor',  { 'message': "It is the name of a directory." } );
        return;        
      }    
      websocket.emit('editor',  { 'action':'prompt', 'message' : "Already exists. Erase it?" }, function(data)
      {
  ...
  fs.writeFile(fullpath, params.content, function(err) {
    ...
    console.log(message);
    if(err) 
      websocket.emit('editor',  { 'message': message });
    else
      websocket.emit('status', {'content' : message });  
    }
  );        
...
function isempty(websocket, fs, params)
{
  ...
  path.exists(fullpath, function (r) { 
    if(!r) { 
      websocket.emit('message', { 'content' : "Dir not found " + fullpath }); 
      return;
    }
    fs.readdir(thepath, function(err, arrfiles)
	  {
		   //websocket.emit('notification',  { 'action': 'update', 'target' : params.target } );
       websocket.emit('message', { 'content' : arrfile.length });
    });   
  });
}
...
function dirinfo(websocket, fs, params)
{
  ...
  websocket.emit('message', { 'content' : sizestr +  ", in " + totalfile + " files" });
}
...
function execute(websocket, fs, params)
{
  var target = params.target;
  ...
  runner.exec(command, function(err, stdout, stderr) { 
      console.log("Terminated. "+ stderr); 
      if(err) websocket.emit('message', { 
          'content' : "Enable to execute " + filename
        }); 
  });
...
function archive(websocket, fs, params)
{
  ...
  runner.exec(command, function(err, stdout, stderr) { 
      console.log("Terminated. "+ stderr); 
      if(err) websocket.emit('message', { 
          'content' : "Enable to create " + zipname
        }); 
      else
        websocket.emit('notification',  { 'action': 'update', 'target' : params.target } );
  });
...
function viewzip(websocket, fs, params)
{
  ...
  try {
    ...
    message(websocket, 'Enable to open '+ zipname + " archive.");
    return;
  } 
	var dirlist = [];  
  var thedate = null;
...
function extractSub(websocket, zipfile, filename, target, keepath, overwrite)
{
  ...
  //console.log("Extract sub " + filename);
  if(zipEntry != false)
  {
    console.log("Extracting file: " + zipEntry.entryName + " to " + targetpath);
    if(!overwrite)
    {
      if(fs.existsSync(targetpath))
      { 
        websocket.emit('confirm', { 'question' : "Already exists. Erase it?" }, function(data)
        {
...
{
console.log(params.archive + " "+ params.entryname);
  var zipname = fs.realpathSync(params.archive);
  var zipfile = new zip(zipname);
  ...
  if(zipEntry != null && zipEntry != false)
  {
    ...
    {
      websocket.emit('message', {'content':'Enable to extract. See the issues page.' });
      return;
    } 
...
function unzip(websocket, fs, params)
{
  ...
  console.log("Unzip " + zipname + " to " + currpath[params.target]);
  var zipfile = new zip(zipname);

  try {
    zipfile.extractAllTo(currpath[target], params.overwrite);
  } catch(e) { 
    websocket.emit('message', {'content':'Enable to extract. See the issues page.' });
    return;
  }  
...
exports.shell = function(websocket, fs, params)
{
	...
	case 'getdir':
		subdir(filepath, target);
    console.log("Explorer shell getdir from " + filepath + " to " + currpath[target]);
		getdir(websocket, fs, currpath[target], params);
		break;
	case 'chdir':  // go to subdir or reload
		chdir(filepath, target);
		getdir(websocket, fs, currpath[target], params);
		break;
	case 'godir':
		godir(filepath, target);
		getdir(websocket, fs, currpath[target], params);
		break;      
  case 'dirup':
		dirup(target);
  ...
  case 'dirinfo':
		...
		break;
  case 'execute':
		execute(websocket, fs, params);
		break;                        
	case 'loadimage':
		console.log("Explorer shell image filepath : " + filepath);
		readImage(websocket, fs, filepath);      
		break;
	case 'thumbnail':
		console.log("Building thumbnail of " + params.imagefile)
		makeThumbnail(websocket, fs, filepath, params);
		break;  
  case 'viewtext':
