...
function getdir(websocket, fs, thepath, params)
{
  ...
  fs.exists(thepath, function (result) {
    if(!result) {
  ...
  try {
    ...
    //websocket.emit('notification', {    });
     ...
     websocket.emit('message', {
        'content' : "Enable to rename " + oldname + " to " + newname + " " + e
      });
  }
}
...
function getcontent(websocket, fs, params)
{
	...
	fs.exists(fullpath, function(here)
	{
		if(here)
		{
    	  ...
    	  if(stats.isDirectory())
    	  {
        	 websocket.emit('message',  { 'content': "Can't edit " + fullpath + ", it is a directory." } );
        	 return;
      	  }
